...
@README.md @claude-local-preferences.md
...
- Config (style-guide URLs, LLM key, WordPress creds) lives in **Script
  Properties**, never in code. See @README.md.

## Static checks

- There's no lint/test tooling. To syntax-check a `.gs` file:
  `cp file.gs /tmp/x.js && node --check /tmp/x.js` — it's ES5-style JS and node
  can't read `.gs` directly. A PostToolUse hook runs this automatically on edits.
