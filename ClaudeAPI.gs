...
var STYLE_GUIDE_SYSTEM_PROMPT_BASE = [
  ...
  '',
  '8. SERIAL/OXFORD COMMAS: This is the one punctuation issue you SHOULD flag, because it needs judgment a regex cannot provide. Follow the rule in the style guide below. If the guide requires the serial (Oxford) comma, flag genuine lists of three or more items that are missing the comma before the final "and"/"or". The "excerpt" must span the ENTIRE list (from the first item through the last) and the "suggestion" must be that same span with the comma added — never just the final two items, or the leading items will be duplicated when the fix is applied. E.g. excerpt "AI, biosecurity and nuclear risk" -> suggestion "AI, biosecurity, and nuclear risk". Only flag actual lists — never a compound sentence with two clauses joined by "and". If the guide forbids the serial comma, do the reverse. Use category "clarity" for these.',
  '',
  ...
  '- Weak link anchor text such as "click here" or "read more" (the deterministic checker already flags these)',
...
var STYLE_GUIDE_OUTPUT_FORMAT = [
  ...
  '  - "suggestion": the exact replacement text for "excerpt", or null if the fix needs human judgment and cannot be expressed as a literal replacement',
  '  - "severity": "warning" or "info"',
  '  - "category": "tone" | "hedging" | "clarity" | "voice" | "precision" | "structure"',
  '',
  'CRITICAL — the fix is applied by replacing the "excerpt" span with "suggestion" verbatim, so:',
  '  - "suggestion" must be a drop-in replacement for EXACTLY the text in "excerpt" — same start and end. Do not include words that fall outside "excerpt" (they get duplicated), and do not drop words that are inside it (they get deleted).',
  '  - "suggestion" must be the literal text the span becomes — NEVER an instruction or recommendation (e.g. not "replace with descriptive link text" or "rephrase this"). If you cannot give literal replacement text, set "suggestion" to null and put the advice in "message".',
  '',
